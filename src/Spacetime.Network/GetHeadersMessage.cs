...
/// </summary>
public sealed class GetHeadersMessage
{
    /// <summary>
    ...
    {
        if (locatorHash.Length != HashSize)
        {
            throw new ArgumentException($"Locator hash must be {HashSize} bytes.", nameof(locatorHash));
        }

        if (stopHash.Length != 0 && stopHash.Length != HashSize)
        {
            throw new ArgumentException($"Stop hash must be empty or {HashSize} bytes.", nameof(stopHash));
        }
    ...
    public byte[] Serialize()
    {
        ...
        var totalLength = HashSize + 1 + (hasStopHash ? HashSize : 0) + 4;
        var buffer = new byte[totalLength];
        var offset = 0;

        LocatorHash.Span.CopyTo(buffer.AsSpan(offset, HashSize));
        offset += HashSize;
        ...
        if (hasStopHash)
        {
            StopHash.Span.CopyTo(buffer.AsSpan(offset, HashSize));
            offset += HashSize;
        }
    ...
    public static GetHeadersMessage Deserialize(ReadOnlyMemory<byte> data)
    {
        ...
        var locatorHash = data.Slice(offset, HashSize);
        offset += HashSize;
